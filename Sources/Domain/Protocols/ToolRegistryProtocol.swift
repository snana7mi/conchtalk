...
nonisolated protocol ToolRegistryProtocol: Sendable {
    /// 当前已注册工具列表。
    ...
    /// 导出符合 OpenAI Chat Completions 协议的 `tools` 数组（带服务器能力过滤）。
    /// 当服务器无可用编码代理时，自动排除 `suggest_agent_connection` 工具定义。
    /// - Parameter capabilities: 服务器能力信息。
